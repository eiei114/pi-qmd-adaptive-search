...
/* ── Helpers ────────────────────────────────────────────────────── */
function classifySeverity(value, warn, crit) {
    if (value >= crit)
        return 'critical';
    if (value >= warn)
        return 'warning';
...
/* ── Bucket analysers ───────────────────────────────────────────── */
function analyzeLearnedAliases(root) {
    const p = paths(root);
...
function analyzePendingSuggestions(root) {
    const p = paths(root);
...
function analyzeEmbedDebt(root, config, qmd) {
    if (!qmd.available) {
        return {
    ...
    let needsEmbeddings = null;
    let embedDebtPercent = null;
    if (needsEmbedMatch) {
...
function analyzeSearchHistory(root) {
    ...
    const lastCompleted = recentJobs.find((job) => ['qmd-search', 'qmd-query'].includes(job.type) && job.status === 'completed');
    const failedSearchJobs = recentJobs.filter((job) => ['qmd-search', 'qmd-query'].includes(job.type) && job.status === 'failed');
 ...
 */
function diagnoseSearchQuality(root, config, qmd) {
    ...
    const severities = buckets.map((b) => b.severity);
