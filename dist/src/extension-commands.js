...
import { adaptiveSearch, recordFeedback, adaptiveStatus, initProject, applyPreset, reviewSuggestions, approveSuggestions, installInstructions, qmdOperationPlan, runQmdOperation, maintenancePlan, runMaintenance } from './index.js';
...
function hasYesFlag(text) {
    return /(^|\s)--yes(\s|$)/.test(String(text || ''));
}
const SEARCH_MODES = ['auto', 'precision', 'recall', 'article', 'project'];
const FEEDBACK_RATINGS = ['good', 'bad'];
const CONFIGURE_PRESETS = ['docs', 'mixed', 'code', 'privacy'];
...
function handleApprove(ctx) {
    return jsonResult(approveSuggestions({ root: ctx.cwd }));
}
function applyConfigurePreset(preset, ctx) {
    const result = applyPreset(ctx.cwd, preset);
    ctx.ui.notify(`qmd adaptive preset applied: ${preset}`, 'info');
    return jsonResult(result);
}
async function handleConfigure(args, ctx) {
    const presetArg = String(args || '').trim();
    if (presetArg)
        return applyConfigurePreset(presetArg, ctx);
    if (ctx.hasUI === false || typeof ctx.ui.select !== 'function') {
        const message = 'Preset selection requires the Pi TUI. For scripts or headless runs, use `qmd-adaptive-search configure --preset docs|mixed|code|privacy`.';
...
function handleQmdOperation(operation, args, ctx, options) {
...
function parseMaintainTargets(args) {
    const trimmed = String(args || '').trim();
    if (!trimmed)
        return undefined;
    return trimmed.split(/\s+/).filter(Boolean);
}
function handleMaintain(args, ctx, options) {
...
export function registerQmdAdaptiveTools(pi) {
    pi.registerTool({
        ...
        parameters: Type.Object({
            ...
            mode: Type.Optional(Type.Union(SEARCH_MODES.map((mode) => Type.Literal(mode)), { description: 'auto, precision, recall, article, or project.' })),
            scopeHint: Type.Optional(Type.Union([Type.String(), Type.Array(Type.String())], {
                description: 'Path/folder hints for soft boosting.'
            })),
            maxResults: Type.Optional(Type.Integer({ minimum: 1, maximum: 30, description: 'Default 10, hard max 30.' }))
        }),
        async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
            return formatAdaptiveSearchToolResult(adaptiveSearch(normalizeSearchToolParams(params), { root: ctx.cwd }));
        }
    });
    pi.registerTool({
        ...
        parameters: Type.Object({
            ...
            rating: Type.Optional(Type.Union(FEEDBACK_RATINGS.map((rating) => Type.Literal(rating)), { description: 'good or bad. Automatic useful-result feedback should use good.' })),
...
export function registerQmdAdaptiveCommands(pi) {
    const colonHandlers = {
        'qmd-a:init': {
