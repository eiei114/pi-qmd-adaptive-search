...
function commandCandidates(config) {
    const candidates = [];
    if (Array.isArray(config.qmdCommand) && config.qmdCommand.length > 0)
        candidates.push(config.qmdCommand);
...
function runCommand(command, args, options = {}) {
    ...
    for (const candidate of bins) {
        ...
        if (!last.error || !['ENOENT', 'EINVAL'].includes(last.error.code))
            return last;
    }
    return last;
}
...
function createPathResolver(root) {
    ...
    return function resolve(raw) {
        ...
        if (fs.existsSync(path.join(root, numberedFolder)))
            return numberedFolder;
        if (byCanonical.size === 0) {
            for (const rel of walkFiles(root))
                byCanonical.set(canonicalPathKey(rel), rel);
        }
...
function detectQmd(config, root = process.cwd()) {
    ...
    for (const candidate of commandCandidates(config)) {
        ...
        if (status.status === 0)
            return { available: true, command: candidate, statusText: status.stdout || status.stderr || '' };
        const version = runCommand(candidate, ['--version'], { cwd: root, timeoutMs: 3000 });
        if (version.status === 0)
            return { available: true, command: candidate, statusText: version.stdout || version.stderr || '' };
...
function parseQmdSearchOutput(output, root, collectionRoots = {}) {
    const results = [];
    const seen = new Set();
    ...
    for (const line of lines) {
        ...
        if (mapped && (typeof configuredRoot !== 'string' || !path.isAbsolute(configuredRoot)))
            continue;
        const allowedRoot = mapped ? configuredRoot : root;
...
function qmdSearch(query, maxResults, config, root = process.cwd(), options = {}) {
