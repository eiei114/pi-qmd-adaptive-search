...
function inferMode(query, requested = 'auto') {
    ...
    if (/まとめ|記事|draft|post|article|essay|note/.test(q))
        return 'article';
    if (/仕様|決定|adr|計画|plan|spec|project|prd|roadmap/.test(q))
        return 'project';
    if (/関連|広く|周辺|similar|related/.test(q))
        return 'recall';
    if (/どこ|場所|file|path|exact/.test(q))
        return 'precision';
    return 'auto';
}
...
function shouldInclude(rel, config) {
    ...
    if (matchesAny(posix, config.excludeGlobs || []))
        return false;
    if (!matchesAny(posix, config.fileGlobs || []))
        return false;
...
function walkFiles(root, config, dir = root, output = []) {
    for (const entry of fs.readdirSync(dir, { withFileTypes: true })) {
        ...
        if (entry.isDirectory())
            walkFiles(root, config, abs, output);
        else if (entry.isFile() && shouldInclude(rel, config))
            output.push(rel);
    }
    return output;
}
    ...
    try {
        ...
        for (const line of lines) {
            ...
            if (terms.some((term) => lower.includes(term)))
                hits.push(line.trim().slice(0, maxChars));
            if (hits.length >= maxPer)
                break;
        }
        return hits;
    }
...
function scoreFile(root, rel, terms, scopeHints, mode, boostEntries) {
    ...
    for (const hint of scopeHints) {
        ...
        if (h && relLower.startsWith(h.replace(/\/$/, ''))) {
            score += 0.25;
            source.push('boost');
            why.push(`scope boost: ${hint}`);
        }
        else if (h && relLower.includes(h)) {
            score += 0.12;
            source.push('boost');
            why.push(`scope keyword: ${hint}`);
        }
    }
    ...
    if (/archive|_archive|old|deprecated/i.test(rel)) {
        score -= 0.12;
        why.push('archive penalty');
    }
    if (mode === 'article' && /\.md$|\.txt$|docs?\//i.test(rel))
        score += 0.08;
    if (mode === 'project' && /prd|spec|adr|readme|plan|docs?\//i.test(rel))
        score += 0.1;
