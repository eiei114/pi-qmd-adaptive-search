...
function addAliases(root, anchors, selectedPaths) {
  ...
  for (const anchor of anchors) {
    ...
    for (const term of pathTerms) if (term !== anchor) existing.add(term);
    if (existing.size > 0) current.aliases[anchor] = Array.from(existing).slice(0, 12);
  }
...
function recordFeedback(input, options = {}) {
  ...
  if (!['good', 'bad'].includes(rating)) throw new Error('rating must be good or bad');
  const selectedPaths = normalizeSelected(input);
  if (selectedPaths.length === 0) throw new Error('selectedPaths or selectedPath is required');
  ...
  if (!matchedSearch && input.force) warnings.push('accepted forced feedback outside recent search with low confidence');
...
function approveSuggestions(options = {}) {
  ...
  for (const item of review.suggestions) {
    if (item.rating !== 'good') continue;
    for (const selectedPath of item.selectedPaths || []) sharedBoosts.boosts[selectedPath] = Number(((sharedBoosts.boosts[selectedPath] || 0) + 0.05).toFixed(3));
    for (const anchor of item.anchors || []) {
      const existing = new Set(sharedAliases.aliases[anchor] || []);
      for (const selectedPath of item.selectedPaths || []) for (const term of tokenize(path.basename(selectedPath, path.extname(selectedPath)))) existing.add(term);
